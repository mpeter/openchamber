...
- `packages/web/server/lib/opencode/lifecycle.js`: OpenCode process lifecycle runtime (startup, restart, readiness, health monitoring). After readiness it warms the most recently used directories (`getWarmupDirectories` dep, sequential and best-effort) because OpenCode initializes each directory lazily on first request and that cost would otherwise be paid by the user's first interactive session open. `OPENCHAMBER_OPENCODE_WARMUP_DIRECTORY_LIMIT` defaults to `4`; set it to `0` to disable only background warmup on constrained hosts, leaving interactive directory initialization unchanged.
