...
`MainLayout` and `VSCodeLayout` each call `useSessionListSync({ isVSCode })` directly and unconditionally, outside Sidebar visibility, responsive, editor, settings, and compact-view branches. The hook selects the real topology inputs, publishes only active-project, current-directory, and selected-session bootstrap demand through `ChildStoreManager`, refreshes topology additions (including all VS Code directories on its first mount), coalesces OpenChamber control events for 500ms, and supplies a memoized complete global active+archived input to authoritative cleanup. The root-level global poller owns the initial global refresh. MainLayout includes available worktrees; VS Code intentionally excludes them. Sidebar-local `session-created` worktree discovery is separate and full-app-only.
...
- Layout demand covers active-project, current-directory, and selected-session directories. The sidebar collection separately publishes expanded and visible directories. Collapsed, off-screen, and unopened directories are represented by the global sessions cache and initialize only when selection or sidebar visibility makes them relevant.
...
- Selected, active-project, expanded, and visible bootstrap demand also protects a store from eviction. This keeps virtualized off-screen directories alive while their owner still needs them; unopened directories have no layout-level background demand and remain represented by the global sessions cache until relevant.
